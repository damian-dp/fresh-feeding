...
const alertVariants = cva(
  "flex items-start gap-3 w-full rounded-md border px-5 py-4 [&>svg]:text-foreground [&>svg]:size-5 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
...
const AlertTitle = React.forwardRef(({ className, ...props }, ref) => (
  <h5
    ref={ref}
    className={cn("font-medium text-base leading-none mt-0.5", className)}
    {...props}
  />
));
