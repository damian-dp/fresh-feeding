...
export function AddDogDialog({ open, onOpenChange }) {
          ...
          .insert({
            ...
            ratios_intake:
              data.goal === "maintain"
                ...
                ? 2
                : data.ratios_intake,
            use_puppy_guidelines: shouldUsePuppy,
          })
          .select()
          .single();
        if (!e1) {
          ...
          // Retry without the new column (in case migration isn't applied yet)
  ...
  const handleNext = () => {
    ...
    // Skip goals step if puppy guidelines are ON and age < 12m
    if (step === 0) {
      const dob = form.getValues("dob");
      const ageMonths = dob ? getDogAgeInMonths(dob) : null;
      const usePuppy = form.getValues("use_puppy_guidelines") ?? true;
      if (ageMonths != null && ageMonths < 12 && usePuppy) {
        setStep(2);
        return;
      }
    }
                ...
                {step === 0 && (
                  <>
                    <div className="hidden md:block">
                      <h2 className="text-3xl font-medium">Add a new dog</h2>
                    </div>
                    ...
                    <div className="flex flex-col gap-6 w-full px-6">
                              ...
                              <FormControl>
                                <Input
                                  ...
                                  className={cn(
                                    ...
                                    form.formState.errors.dog_name &&
                                      "border-destructive bg-destructive/10 text-destructive"
                                  )}
                                  onChange={(e) => {
                                    field.onChange(e);
                                    if (e.target.value) {
                                      clearFieldError("dog_name");
                                    }
                                  }}
                                />
                              </FormControl>
                              ...
                              <FormItem className="w-full">
                                ...
                                <Input
                                  ...
                                  onBlur={(e) => {
                                    ...
                                    if (naturalDate && isValid(naturalDate)) {
                                ...
                                />
                                {form.formState.errors.dob &&
                                  form.getFieldState("dob").isTouched && (
                                    <FormMessage>
                                ...
                                <FormControl>
                                  <Input
                                    ...
                                    className={cn(
                                      ...
                                      form.formState.errors.weight_metric &&
                                    ...
                                    onBlur={(e) => {
                                      ...
                                      if (!isNaN(value)) {
                                        ...
                                        handleWeightChange(rounded, "metric");
                                      }
                                    }}
                                  />
                                </FormControl>
                      ...
                      {(() => {
                        const dobVal = form.watch("dob");
                        const ageMonths =
                          dobVal instanceof Date
                            ? getDogAgeInMonths(dobVal)
                            : null;
                        const showPuppySwitch =
                          ageMonths != null && ageMonths < 12;
                        if (!showPuppySwitch) return null;
                        return (
                          <Label className="border border-input bg-background hover:bg-background/50 items-start gap-3 rounded-xl p-4 has-[[aria-checked=true]]:border-input has-[[aria-checked=true]]:bg-background transition-colors">
                            <Switch
                              checked={
                                form.watch("use_puppy_guidelines") ?? true
                              }
                            ...
                            <div className="grid gap-1.5 font-normal">
                              ...
                              <p className="text-muted-foreground text-sm">
                                Puppies (including small/working breeds) may
                                need adjustments during growth spurts. Monitor
                                body condition and adjust as needed.
                              </p>
                            </div>
                          </Label>
                        );
                ...
                {step === 1 && (
                  <>
                    <div className="flex flex-col gap-1 justify-center items-center">
                      <h2 className="text-3xl font-medium">What's your goal</h2>
                      <h2 className="text-3xl font-medium">
                        for {form.getValues("dog_name")}?
                      </h2>
                    </div>
                    {/* If puppy guidelines are ON and <12m, this step will be skipped; no need to show the switch here */}
                    <FormField
                      control={form.control}
                      name="goal"
                      render={({ field }) => (
                            ...
                            {[
                              ...
                              >
                                <Button
                                  ...
                                  onClick={() =>
                                    form.watch("use_puppy_guidelines") &&
                                    (getDogAgeInMonths(form.getValues("dob")) ??
                                      0) < 12
                                      ? null
                    ...
                    <div className=" flex flex-col gap-1 justify-center items-center">
                      {form.getValues("goal") === "maintain" && (
                        <p className="text-xl px-4 font-normal text-muted-foreground/60 text-center leading-relaxed">
                          ...
                          maintain their current weight of{" "}
                          <span className=" text-foreground">
                            {form.getValues("weight_metric")}
                            kg
                          </span>
                          , they should be fed{" "}
                          <span className=" text-foreground">
                      ...
                      {form.getValues("goal") === "gain" && (
                        <p className="text-xl px-4 font-normal text-muted-foreground/60 text-center leading-relaxed">
                          To help{" "}
                          <span className=" text-foreground">
                            {form.getValues("dog_name")}
                          </span>{" "}
                          increase their current weight of{" "}
                          <span className=" text-foreground">
                            {form.getValues("weight_metric")}
                            kg
                          </span>
                          ...
                          <span className=" text-foreground">
                            {Math.round(
                              (3 / 100) * form.getValues("weight_metric") * 1000
                            )}
                            g
                          </span>{" "}
                          per day.
                      ...
                      {form.getValues("goal") === "lose" && (
                        <p className="text-xl px-4 font-normal text-muted-foreground/60 text-center leading-relaxed">
                          To help{" "}
                          <span className=" text-foreground">
                            {form.getValues("dog_name")}
                          </span>{" "}
                          decrease their current weight of{" "}
                          <span className=" text-foreground">
                            {form.getValues("weight_metric")}
                            kg
                          </span>
                          ...
                          <span className=" text-foreground">
                            {Math.round(
                              (2 / 100) * form.getValues("weight_metric") * 1000
                            )}
                            g
                          </span>{" "}
                          per day.
                      ...
                      <div className="relative w-32 h-32 rounded-full text-muted-foreground overflow-hidden bg-muted/40 border border-border flex items-center justify-center">
                        {avatarPreview || avatarUrl ? (
                          ...
                          <div className="text-[2.5rem] font-medium">
                            {form.getValues("dog_name")[0].toUpperCase()}
                          </div>
                        )}
                      </div>
                      <div className="flex flex-col gap-1 justify-center items-center">
                        <h2 className="text-3xl font-medium">Welcome to the</h2>
                        <h2 className="text-3xl font-medium">
                          pack {form.getValues("dog_name")}
                        </h2>
                      </div>
                    </div>
                  </>
