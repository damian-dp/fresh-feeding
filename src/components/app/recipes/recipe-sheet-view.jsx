...
export function RecipeSheetView({
  ...
  useEffect(() => {
    ...
    if (recipe?.batch_size) {
      ...
      if (dog) {
        const dailyIntake =
          dog.weight_metric * (getCurrentIntakePercent(dog) / 100);
      ...
      <div className="flex flex-col gap-8 p-8 border-t border-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-12">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <BadgeStack
                ...
                sublabel={(() => {
                  const dog = dogs.find((d) => d.dog_id === localRecipe.dog_id);
                  if (!dog) return "Unknown";
                  const grams = getCategoryGrams(dog, dog.ratios_muscle_meat);
                  return (
                    <>
                      {grams}g{" "}
                      <span className="text-muted-foreground">
                        / {Math.round(dog.ratios_muscle_meat * 100)}%
                      </span>
                    </>
                  );
                })()}
            ...
            <div
              className={
                getIngredientsByCategory(localRecipe.recipe_ingredients, 1)
                  .length > 0
                  ? "border-t"
                  : ""
              }
            >
              {getIngredientsByCategory(localRecipe.recipe_ingredients, 1).map(
                (item) => (
                  <div
                    key={item.ingredient_id}
                    className="flex items-center justify-between border-b py-4"
                  >
                    <span>{item.ingredients?.ingredient_name}</span>
                    {/* <span className="text-muted-foreground">
                                        {Math.round(
          ...
          <div className="flex flex-col gap-6">
            <BadgeStack
              ...
              sublabel={(() => {
                const dog = dogs.find((d) => d.dog_id === localRecipe.dog_id);
                if (!dog) return "Unknown";
                const grams = getCategoryGrams(dog, dog.ratios_plant_matter);
                return (
                  <>
                    {grams}g{" "}
                    <span className="text-muted-foreground">
                      / {Math.round(dog.ratios_plant_matter * 100)}%
                    </span>
                  </>
                );
              })()}
            ...
            <div
              className={
                getIngredientsByCategory(localRecipe.recipe_ingredients, 2)
                  .length > 0
                  ? "border-t"
                  : ""
              }
            >
              {getIngredientsByCategory(localRecipe.recipe_ingredients, 2).map(
                (item) => (
                  <div
                    key={item.ingredient_id}
                    className="flex items-center justify-between border-b py-4"
          ...
          <div className="flex flex-col gap-6">
            <BadgeStack
              ...
              sublabel={(() => {
                const dog = dogs.find((d) => d.dog_id === localRecipe.dog_id);
                if (!dog) return "Unknown";
                const grams = getCategoryGrams(dog, dog.ratios_liver);
                return (
                  <>
                    {grams}g{" "}
                    <span className="text-muted-foreground">
            ...
            <div
              className={
                getIngredientsByCategory(localRecipe.recipe_ingredients, 3)
                  .length > 0
                  ? "border-t"
                  : ""
              }
            >
              {getIngredientsByCategory(localRecipe.recipe_ingredients, 3).map(
                (item) => (
                  <div
                    key={item.ingredient_id}
                    className="flex items-center justify-between border-b py-4"
          ...
          <div className="flex flex-col gap-6">
            <BadgeStack
              ...
              sublabel={(() => {
                const dog = dogs.find((d) => d.dog_id === localRecipe.dog_id);
                if (!dog) return "Unknown";
                const grams = getCategoryGrams(dog, dog.ratios_secreting_organ);
                return (
                  <>
                    {grams}g{" "}
                    <span className="text-muted-foreground">
                      / {Math.round(dog.ratios_secreting_organ * 100)}%
                    </span>
                  </>
                );
              })()}
            ...
            <div
              className={
                getIngredientsByCategory(localRecipe.recipe_ingredients, 4)
                  .length > 0
                  ? "border-t"
                  : ""
              }
            >
              {getIngredientsByCategory(localRecipe.recipe_ingredients, 4).map(
                (item) => (
                  <div
                    key={item.ingredient_id}
                    className="flex items-center justify-between border-b py-4"
          ...
          <div className="flex flex-col gap-6 md:col-span-2">
            ...
            <div
              className={
                getIngredientsByCategory(localRecipe.recipe_ingredients, 5)
                  .length > 0
                  ? "border-t"
                  : ""
              }
            >
              {getIngredientsByCategory(localRecipe.recipe_ingredients, 5).map(
                (item) => (
                  <div
                    key={item.ingredient_id}
                    className="flex items-center justify-between border-b py-4"
