...
export function BoneCalculator({ open, onOpenChange }) {
  ...
  const boneIngredients =
    ingredients?.filter(
      (ingredient) => ingredient?.bone_percent && ingredient.bone_percent > 0
    ) || [];
  ...
  const calculateBoneRequirements = () => {
    ...
    const dailyIntakeGrams =
      (getCurrentIntakePercent(dogDetails) / 100) *
      ...
      <DialogContent className="">
        <DialogHeader>
          ...
          <DialogDescription className="hidden">
            Calculate how much bone-in food your dog needs based on their daily
            requirements.
          </DialogDescription>
        </DialogHeader>
              ...
              >
                <div className="py-4 px-5 font-normal bg-muted/8 border border-border rounded-sm flex flex-col gap-2 text-muted-foreground/90">
                  <p className="text-xl leading-[1.8]">
                    ...
                    needs{" "}
                    <span className="text-foreground">
                      {requirements.dailyIntake.toLocaleString()}g
                    </span>{" "}
                    of food per day, including{" "}
                    <span className="text-foreground">
                      {requirements.boneRequirement.toLocaleString()}g
                    </span>{" "}
                    of bone content.
                  </p>
                  <p className="text-xl leading-[1.8]">
                    To meet this requirement, feed approximately{" "}
                    <span className=" text-foreground">
                      {requirements.ingredientRequired.toLocaleString()}g
                    </span>{" "}
                    of{" "}
                    <span className="text-foreground">
